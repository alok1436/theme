...
(function($) {
    ...
    redux.field_objects.css_layout.mainID = '';
    ...
    redux.field_objects.css_layout.modInit = function(el) {
        ...
        // Get the main ID from field.
        redux.field_objects.css_layout.mainID = el.attr('data-id');
        ...
        el.find('#' +  redux.field_objects.css_layout.mainID + ' .redux-color-css-layout-border' ).wpColorPicker({
            change: function( event, ui ) {
                ...
                el.find('#' + redux.field_objects.css_layout.mainID).find('.redux-css-layout-border').css({'border-color': color});
            }
        });
        ...
        el.find( '.redux-style-css-layout-border' ).on('change', function(e) {
            ...
            el.find('#' + redux.field_objects.css_layout.mainID).find('.redux-css-layout-border').css({'border-style': style});
        });
        ...
        el.find( '.css-layout-input' ).on('change', function(e) {
            ...
            if ($(this).hasClass('redux-css-layout-input-radius')) {
                ...
                el.find('#' + redux.field_objects.css_layout.mainID).find('.redux-css-layout-border').css({'border-radius': radius});
            }
        });     
    };
        ...
        $.each(levelArr, function(idx, val) {
            if (selector.hasClass('redux-css-' + val)) {
                $.each(posArr, function(index, value) {
                    finalArr[index] = el.find('#' + redux.field_objects.css_layout.mainID).find('.redux-css-' + val + '-' + value).val();
                    finalArr[index] = (finalArr[index] === '') ? '0' : finalArr[index];
                });

                var shorthand = finalArr[0] + ' ' + finalArr[1] + ' ' + finalArr[2] + ' ' + finalArr[3];
                el.find('#' + redux.field_objects.css_layout.mainID).find('.redux-css-' + val + '-shorthand').val(shorthand);
            }
        });
    };
    ...
    redux.field_objects.css_layout.getUnit = function(str, el){
        ...
        var unitArr = $('#' + redux.field_objects.css_layout.mainID).data('units');
    ...
    redux.field_objects.css_layout.defUnitVal = function (selector, el) {
        ...
        // Set mainID
        var mainID = redux.field_objects.css_layout.mainID;
        var unitVal;
