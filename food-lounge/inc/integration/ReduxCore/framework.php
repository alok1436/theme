...
// Fix for the GT3 page builder: http://www.gt3themes.com/wordpress-gt3-page-builder-plugin/
    ...
    if ( ! class_exists( 'ReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once get_template_directory() . '/inc/integration/ReduxCore/inc/class.redux_instances.php';
        require_once get_template_directory() . '/inc/integration/ReduxCore/inc/lib.redux_instances.php';
    }
...
// Don't duplicate me!
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        // Redux CDN class
        require_once get_template_directory() . '/inc/integration/ReduxCore/inc/class.redux_cdn.php';

        // Redux API class  :)
        require_once get_template_directory() . '/inc/integration/ReduxCore/inc/class.redux_api.php';

        // General helper functions
        require_once get_template_directory() . '/inc/integration/ReduxCore/inc/class.redux_helpers.php';

        // General functions
        require_once get_template_directory() . '/inc/integration/ReduxCore/inc/class.redux_functions.php';
        require_once get_template_directory() . '/inc/integration/ReduxCore/inc/class.p.php';

        require_once get_template_directory() . '/inc/integration/ReduxCore/inc/class.redux_filesystem.php';

        require_once get_template_directory() . '/inc/integration/ReduxCore/inc/class.redux_admin_notices.php';

        // ThemeCheck checks
        //require_once get_template_directory() . '/inc/integration/ReduxCore/inc/themecheck/class.redux_themecheck.php';
        ...
        class ReduxFramework {
            ...
            public static function init() {
                $dir = Redux_Helpers::cleanFilePath( get_template_directory() . '/inc/integration/ReduxCore' );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( esc_html__( 'Options panel created using %1$s', 'food-lounge' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'food-lounge' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'food-lounge' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'food-lounge' );
                }
...
// __construct()
            ...
            private function _internationalization() {
                 ...
                 * @param string     'food-lounge'  text domain
                 */
                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'food-lounge' );
                ...
                load_textdomain( 'food-lounge', self::$_dir . 'languages/' . $locale . '.mo' );
            }
...
// get_instance()
            ...
            private function _tracking() {
                if ( file_exists( get_template_directory() . '/inc/integration/ReduxCore/inc/tracking.php' ) ) {
                    require_once get_template_directory() . '/inc/integration/ReduxCore/inc/tracking.php';
            ...
            public function _admin_bar_menu() {
                ...
                if ( $menu ) {
                    ...
                    $nodeargs = array(
                        ...
                        'title' => $title,
                        // $theme_data->get( 'Name' ) . " " . esc_html__( 'Options', 'food-lounge' ),
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'food-lounge' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'food-lounge' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_html__( 'moving the mouse over', 'food-lounge' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_html__( 'clicking', 'food-lounge' );
                    }

                    // Construct message
                    $msg = sprintf( esc_html__( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'food-lounge' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_html__( 'Hints', 'food-lounge' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_html__( 'Default', 'food-lounge' ) . ": " . substr( $default_output, 0, - 2 );
                }
...
// _register_settings()
            ...
            private function _register_extensions() {
                $path    = get_template_directory() . '/inc/integration/ReduxCore/inc/extensions/';
...
//                }
            ...
            public function ajax_save() {
                ...
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" ) ) {
                    json_encode( array(
                        'status' => esc_html__( 'Invalid security credential, please reload the page and try again.', 'food-lounge' ),
                        'action' => 'reload'
                    ) );
                    die();
                }
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'food-lounge' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
