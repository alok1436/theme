...
class FOOD_Base
{
    ...
    public static function getPageTitle(){
        ...
        if (!is_archive()){
            ...
            if(is_page()) :
                /* custom title. */
                if(isset($food_meta->_food_page_title_text) && !empty($food_meta->_food_page_title_text)):
                    echo esc_attr($food_meta->_food_page_title_text);
                else :
                    the_title();
                endif;
            /* blog */
            elseif (is_front_page() || is_home()):
                esc_html_e( 'Blog', 'food-lounge' );
            /* search */
            elseif (is_search()):
                printf( esc_html__( 'Search Results for: %s', 'food-lounge' ), '<span>' . get_search_query() . '</span>' );
            /* 404 */ 
            elseif (is_404()):
                esc_html_e( '404', 'food-lounge');
            /* other */
            else :
                the_title();
            endif;
            ...
            /* author. */
            elseif ( is_author() ) :
                printf( esc_html__( 'Author: %s', 'food-lounge' ), '<span class="vcard">' . get_the_author() . '</span>' );
            /* date */
            elseif ( is_day() ) :
                printf( esc_html__( 'Day: %s', 'food-lounge' ), '<span>' . get_the_date() . '</span>' );
            elseif ( is_month() ) :
                printf( esc_html__( 'Month: %s', 'food-lounge' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'food-lounge' ) ) . '</span>' );
            elseif ( is_year() ) :
                printf( esc_html__( 'Year: %s', 'food-lounge' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'food-lounge' ) ) . '</span>' );
            /* post format */
            elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
                esc_html_e( 'Asides', 'food-lounge' );
            elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
                esc_html_e( 'Galleries', 'food-lounge');
            elseif ( is_tax( 'post_format', 'post-format-image' ) ) :
                esc_html_e( 'Images', 'food-lounge');
            elseif ( is_tax( 'post_format', 'post-format-video' ) ) :
                esc_html_e( 'Videos', 'food-lounge' );
            elseif ( is_tax( 'post_format', 'post-format-quote' ) ) :
                esc_html_e( 'Quotes', 'food-lounge' );
            elseif ( is_tax( 'post_format', 'post-format-link' ) ) :
                esc_html_e( 'Links', 'food-lounge' );
            elseif ( is_tax( 'post_format', 'post-format-status' ) ) :
                esc_html_e( 'Statuses', 'food-lounge' );
            elseif ( is_tax( 'post_format', 'post-format-audio' ) ) :
                esc_html_e( 'Audios', 'food-lounge' );
            elseif ( is_tax( 'post_format', 'post-format-chat' ) ) :
                esc_html_e( 'Chats', 'food-lounge' );
            /* woocommerce */
    ...
    public static function getBreadCrumb($separator = '') {
        ...
        /* search */
        if( is_search() ){ echo '<li>'.esc_html__("Search", 'food-lounge').'</li>'; }
        /* date */
        if( is_year() ){ echo '<li>'.get_the_time('Y').'</li>'; }
        /* 404 */
        if( is_404() ) {
            echo '<li>'.esc_html__("404 - Page not Found", 'food-lounge').'</li>';
        }
        /* archive */
